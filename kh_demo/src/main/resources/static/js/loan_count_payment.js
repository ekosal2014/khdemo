...
function loanerGetMaxId(json){	
		    ...
		    if (json.object.loanObject.loan_type == '3'){
		    	...
		    	$('#c_first_date_txt').val(moment(json.object.loanObject.start_date).format('DD/MM/YYYY'));	
		    	$("#c_total_money_txt").val(Common.numberWithComma(json.object.loanObject.total_money) +' ៛');
		    	$('#c_total_money_kh').val(Common.khmerMoney(json.object.loanObject.total_money) + " ៛");	
		    	$('#c_start_date_txt').val(moment(json.object.loanObject.start_date).format('DD/MM/YYYY'));
		    	$('#c_time_payment').val(Common.numberWithComma(json.object.loanObject.time) + ' ដង');
		    	$('#c_type_payment_txt').val(a);
		    	$('#c_money_one_payment').val(Common.numberWithComma(json.object.loanPaymentList[0].prak_derm) + ' ៛');
		   ...
		   $.each(json.object.loanPaymentList,function(index,value){
				...
				if (value.txt == '9'){
					...
					dt      = moment(value.end_dt).format('DD/MM/YYYY');
				...
				if (json.object.loanObject.money_type == 'R'){
			    	//total_rate  = Common.numberWithComma(Common.ConvertZeroTwoDigit(value.total_rate +''))  + money_type;
			    	prak_derm   = Common.numberWithComma(Common.ConvertZeroTwoDigit(value.prak_derm  +''))  + money_type;
			    	//total_left  = Common.numberWithComma(Common.ConvertZeroTwoDigit(value.total_left +''))  + money_type;
			    	
				}else{
			    	//total_rate  = Common.numberWithComma(value.total_rate +'') + money_type;
			    	prak_derm   = Common.numberWithComma(value.prak_derm)      + money_type;
			    	//total_left  = Common.numberWithComma(value.total_left +'') + money_type;
...
function checkForPayment(){
	...
	/*if ($('#payment_all').prop('checked') == true){
		...
		var tbl = '<tr style="display:;" class="">'
			+ 	'<td>'						
...
function loanPaymentSaveUpdate(){
	...
	$('.tbl_input_pop table#prak-derm-payment tbody tr.payment').each(function(index,value){
		/*if ($(this).find('input[type=checkbox]').prop('checked') == true){
		 ...
		 //var rate      = $(this).find('.popup_total_rate').val().replace(/[​\u202f\៛\$\,]/g,'').trim();
